...
const SYNONYMS = {
  ...
  delivery_planned_datetime_range:    ["date de livraison","date livraison","date de reception","date reception","delivery date","date livraison souhaitee","date de livraison souhaitee","eta livraison"],
  delivery_company:                   ["societe destinataire","societe livraison","raison sociale destinataire","entreprise destinataire","societe","raison sociale","company","delivery company"],
...
// Spacefill keeps a single delivery phone number. A column explicitly about a landline
// or a fax must not claim it — otherwise it wins over the mobile column that belongs
// there, and the operator loses the number that actually gets called.
const LANDLINE_OR_FAX = /\bfixe\b|\bfax\b|standard|bureau/i;
const EXCLUDED_WORDS = {
  delivery_phone_number: LANDLINE_OR_FAX,
  pickup_phone_number: LANDLINE_OR_FAX,
  // The carrier's phone is not a fallback bin for the recipient's landline either.
  transport_phone_number: LANDLINE_OR_FAX,
};
...
function score(headerNorm, fieldKey, syns, historyCount, fuzzyHistory, valueField, rawHeader) {
  const excluded = EXCLUDED_WORDS[fieldKey];
  if (excluded && excluded.test(rawHeader || headerNorm)) return 0;
...
export function suggestMappings(headers, spacefillFields, history = [], sampleRows = []) {
  ...
  return headers.map((header, colIndex) => {
    ...
    for (const field of spacefillFields) {
      ...
      const s = score(norm, field.field_key, syns, exactHistory[field.field_key] || 0, fuzzyHistory[field.field_key] || 0, valueField, header);
